...
const a1 = ["bici", "coche"];
const a2 = ["coche", "bici", "muñeca"];
const a3 = ["bici", "pc"];

// const a4 = ['bici', 'coche', 'muñeca', 'pc']
...
function getGiftsToRefill(a1: string[], a2: string[], a3: string[]) {
  ...
  const giftsToRefill = new Set();

  for (const gift of gifts) {
    const isInA1 = a1Set.has(gift);
    const isInA2 = a2Set.has(gift);
    const isInA3 = a3Set.has(gift);

    const count = Number(isInA1) + Number(isInA2) + Number(isInA3);
    if (count === 1) {
      giftsToRefill.add(gift);
    }
  }

  return Array.from(giftsToRefill);
}
